...
class SailfishReduceHelper {
  ...
  static class IMergerHelper {
    ...
    IMergerHelper(int partition, JobConf jc, String jobId) {
...
*/
      ...
      // !#! Fix me...replace localhost with AM-host
    ...
    DataInputStream startIMerger() throws IOException {
      ...
      // !#! Fix me...make workbuilder port as a member variable?
      String workbuilderPort = System.getenv("SAILFISH_WORKBUILDER_PORT");
  ...
  static class SailfishReduceValuesIterator<KEY, VALUE> implements
    ...
    public SailfishReduceValuesIterator(int partition,
      ...
      this.jobId = hadoopJob.get("sailfish.mapred.debug_job.id", null);
      if (this.jobId == null)
        this.jobId = hadoopJob.get("mapred.job.id");

      this.imergerHelper = new IMergerHelper(partition, conf, jobId);
